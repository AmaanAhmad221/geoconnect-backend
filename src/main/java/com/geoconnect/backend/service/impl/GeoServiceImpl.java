...
public class GeoServiceImpl implements GeoService {
	...
	public PageResponse<ServiceResponse> getAllServices(Pageable pageable) {
		Page<ServiceResponse> page = serviceRepository.findByAvailableTrue(pageable).map(ServiceResponse::fromService);
		return PageResponse.fromPage(page);
	}

	@Override
	public PageResponse<ServiceResponse> getServicesByCategory(ServiceCategory category, Pageable pageable) {
		Page<ServiceResponse> page = serviceRepository.findByCategoryAndAvailableTrue(category, pageable)
				.map(ServiceResponse::fromService);
		return PageResponse.fromPage(page);
	}

	@Override
	public PageResponse<ServiceResponse> getServicesByCity(String city, Pageable pageable) {
		Page<ServiceResponse> page = serviceRepository.findByCityAndAvailableTrue(city.toLowerCase(), pageable)
				.map(ServiceResponse::fromService);
		return PageResponse.fromPage(page);
	}

	@Override
	public PageResponse<ServiceResponse> searchServices(String keyword, Pageable pageable) {
		Page<ServiceResponse> page = serviceRepository.searchByKeyword(keyword, pageable)
				.map(ServiceResponse::fromService);
		return PageResponse.fromPage(page);
	}

	@Override
	public PageResponse<ServiceResponse> searchWithFilters(String city, ServiceCategory category, Double minPrice,
			Double maxPrice, Pageable pageable) {
		Page<ServiceResponse> page = serviceRepository.searchWithFilters(city, category, minPrice, maxPrice, pageable)
				.map(ServiceResponse::fromService);
		return PageResponse.fromPage(page);
	}
	...
	public List<ServiceResponse> getAllServices() {
		return serviceRepository.findByAvailableTrue().stream().map(ServiceResponse::fromService)
	...
	public List<ServiceResponse> getMyServices(String username) {
		User provider = userRepository.findByUsername(username)
				.orElseThrow(() -> new ResourceNotFoundException("User", "username", username));
		return serviceRepository.findByProvider(provider).stream().map(ServiceResponse::fromService)
				.collect(Collectors.toList());
	}
}